...
class multiSimulation(Simulation):
    ...
    def simulation(self):
        ...
        PgridMax = []
        hrl = []
        self.hrlTestResult = pd.DataFrame()
        totalReward = 0
            ...
            while not terminal:
                ...
                if month == 0:
                    for level in range(1,10):
                        if actions == 0:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "BESS"
                        elif actions == 1:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc1 1"
                        elif actions == 2:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc1 2"
                        elif actions == 3:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc1 3"
                        elif actions == 4:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc2 1"
                        elif actions == 5:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc2 2"
                        elif actions == 6:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc2 3"
                        elif actions == 7:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc3 1"
                        elif actions == 8:
                            self.hrlTestResult[totalState["sampleTime"]][str(level)] = "Lc3 2"
        ...
        self.hrlTestResult.to_csv('output.csv', index=True)
        # print('Agent average episode reward: ', sum(TotalReward)/len(TotalReward) ) 
        # print('reward: ', TotalReward ) 


    def hrlSimulation(self):
        self.hrlTestResult = pd.DataFrame([],columns=['timeBlock','level 1','level 2','level 3','level 4','level 5','level 6','level 7','level 8','level 9'])
        for i in range(96):
            self.hrlTestResult.loc[i,'timeBlock'] = i
        ...
        while not terminal:
            ...
            totalState = self.environment.totalState
            if actions == 0:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "BESS"
            elif actions == 1:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc1 1"
            elif actions == 2:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc1 2"
            elif actions == 3:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc1 3"
            elif actions == 4:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc2 1"
            elif actions == 5:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc2 2"
            elif actions == 6:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc2 3"
            elif actions == 7:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc3 1"
            elif actions == 8:
                self.hrlTestResult.loc[totalState["sampleTime"]]['level '+str(totalState["order"])] = "Lc3 2"
        self.hrlTestResult.to_csv('output.csv', index=True)
    ...
    def plotunintResult(self,id,month):
        ...
        unint1.fixloadPower(month=month)
